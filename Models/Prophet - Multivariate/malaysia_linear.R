...
# model
Malaysia_model <- prophet(df = NULL, growth = "linear", yearly.seasonality = FALSE,
...
Malaysia_Prophet_multi_linear_Preds <- bind_cols(test, Malaysia_future_preds) %>% 
  rename(preds = yhat) %>% 
  mutate(
...
Malaysia_metrics <- Malaysia_Prophet_multi_linear_Preds %>% 
  covid_metrics(new_cases, estimate = preds)

Malaysia_metrics

Malaysia_Prophet_multi_linear <- pivot_wider(Malaysia_metrics, names_from = .metric, values_from = .estimate) %>% 
  mutate(
    location = "Malaysia"
  ) %>% 
  select(
...
save(Malaysia_Prophet_multi_linear, Malaysia_Prophet_multi_linear_Preds,
     file = "Models/Prophet - Multivariate/results_linear/Malaysia_linear_metrics.rda")
