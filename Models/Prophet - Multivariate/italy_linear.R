...
# model
Italy_model <- prophet(df = NULL, growth = "linear", yearly.seasonality = FALSE,
...
Italy_Prophet_multi_linear_Preds <- bind_cols(test, Italy_future_preds) %>% 
  rename(preds = yhat) %>% 
  mutate(
...
Italy_metrics <- Italy_Prophet_multi_linear_Preds %>% 
  covid_metrics(new_cases, estimate = preds)

Italy_metrics

Italy_Prophet_multi_linear <- pivot_wider(Italy_metrics, names_from = .metric, values_from = .estimate) %>% 
  mutate(
    location = "Italy"
  ) %>% 
  select(
...
save(Italy_Prophet_multi_linear, Italy_Prophet_multi_linear_Preds,
     file = "Models/Prophet - Multivariate/results_linear/Italy_linear_metrics.rda")
