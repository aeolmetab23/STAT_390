...
Australia_Prophet_multi_linear <- pivot_wider(Australia_metrics, names_from = .metric, values_from = .estimate) %>% 
  mutate(
    location = "Australia"
  ) %>% 
  select(
...
save(Australia_Prophet_multi_linear, Australia_preds,
     file = "Models/Prophet - Multivariate/results_linear/Australia_linear_metrics.rda")
