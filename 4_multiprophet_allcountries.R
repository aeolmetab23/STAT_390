...
data_big <- data_big %>%
  ...
  mutate(icu_patients = impute_knn(data_big,
                                   column = "icu_patients",
  ...
  mutate(stringency_index = impute_knn(data_big,
...
prophet_mp_fits <- c()
components <- c()
prophet_mp_results <- tibble(country = c(), rmse = c(), mae = c(), mase = c())
# main loop
for (i in our_countries) {
  ...
  country_big_train <- country %>% filter(ds <= "2023-03-19")
  country_big_test <- country %>% filter(ds > "2023-03-19")
  ...
  # making future df
  future_country <- make_future_dataframe(mpfit_country, periods = 42)
  ...
  # save out results
  prophet_mp_results <- prophet_mp_results %>% 
  ...
  save(prophet_mp_results, prophet_mp_fits, components, file = "results/prophet_mp.rda")
  
}

load("results/prophet_mp.rda")
